...
- `update.yml` — production-обновление каждые 10 минут, кэш Xray/Hysteria, сборка подписки.
- `update-ru.yml` (предложение) — опциональная российская точка проверки, **только** если есть self-hosted runner в РФ (см. `docs/RUSSIAN-PROBE-PLAN.md`). Без инфраструктуры этот файл игнорируется.
...
4. Если есть российский runner и нужен второй замер, скопируйте `workflow-templates/update-ru.yml` → `.github/workflows/update-ru.yml` и задайте лейбл runner `ru` + secret `RU_PROBE_TOKEN` для подписи результатов.
