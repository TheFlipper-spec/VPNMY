...
Названия узлов формируются заново и одинаково отображаются в клиентах: страна, категория, HTTPS-задержка и короткий ID. Исходные рекламные названия не попадают в итоговый профиль. `subscription.txt` содержит только импортируемые VPN URI: это сохраняет совместимость с клиентами и с активной проверкой количества узлов в GitHub Actions.
...
vpnmy/artifacts.py            # проверка согласованности FL1PVPN/raw/stats
...
Активная проверка считает строками только узлы, потому публикация формирует `subscription.txt` без служебных заголовков. Дополнительно команда `python -m vpnmy.artifacts --production` проверяет строгий Base64, соответствие raw-файла, уникальность URI, связь каждого URI с `stats.json` и двойную HTTPS-проверку Xray. GitHub App Arena не может изменять `.github/workflows/` без отдельного разрешения, поэтому вариант workflow с этой подробной диагностикой находится в `workflow-templates/update.yml`, а CI — в `workflow-templates/ci.yml`. При необходимости владелец может скопировать их через веб-интерфейс GitHub.
...
`--skip-deep-check` не является production-проверкой и разрешён только вместе с `--dry-run`: диагностический TCP-набор не может перезаписать опубликованную подписку.

Текущие опубликованные файлы можно отдельно проверить командой `python -m vpnmy.artifacts`.
