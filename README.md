...
Названия узлов формируются заново и одинаково отображаются в клиентах: страна, категория, HTTPS-задержка и короткий ID. Исходные рекламные названия не попадают в итоговый профиль. В `subscription.txt` и `FL1PVPN` публикуются только URI узлов: так совпадает число строк с `stats.total`, и текущий GitHub Actions может закоммитить обновление. Название сервиса и статус живут в `stats.json` и на странице проекта.
